...
import { Broker, BrokerStatus, getAllBrokers } from '@src/index';
...
describe('getAllBrokers', () => {
  it('must return a list of brokers with expected properties', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/cvm/corretoras/v1'), {
  ...
  it('must throw an error if fetch fails', async () => {
    ...
    await expect(getAllBrokers()).rejects.toThrow('Internal Server Error');
  });
  ...
  it('must throw an error if fetch is aborted', async () => {
    ...
    await expect(getAllBrokers()).rejects.toThrow(
      'Tempo limite excedido ao buscar todas as corretoras',
    );
  });
});
