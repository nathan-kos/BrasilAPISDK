import { getBaseUrl, getTimeout } from '@src/shared/config/ApiConfig';
import { Uf } from '@src/shared/enums/uf';
import { AppError } from '@src/shared/exceptions/AppError';
import { handleResponseError } from '@src/shared/exceptions/HandlerResponseError';
import { ServerError } from '@src/shared/exceptions/ServerError';
import { TimeoutError } from '@src/shared/exceptions/TimeoutError';
import { State } from '../entities/state';

async function getStateDataByAcronym(acronym: Uf): Promise<State> {
  const url = `${getBaseUrl()}/ibge/uf/v1/${acronym}`;

  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), getTimeout());

    const response = await fetch(url, {
      signal: controller.signal,
    });
    clearTimeout(timeoutId);

    if (!response.ok) {
      await handleResponseError(response);
    }

    const rawState = (await response.json()) as RawState;

    const state = {
      id: rawState.id,
      acronym: rawState.sigla as Uf,
      name: rawState.nome,
      region: {
        id: rawState.regiao.id,
        acronym: rawState.regiao.sigla,
        name: rawState.regiao.nome,
      },
    };

    return state;
  } catch (error) {
    if ((error as Error).name === 'AbortError') {
      throw new TimeoutError('The request timed out while fetching state data');
    }
    if (error instanceof AppError) {
      throw error;
    } else {
      throw new ServerError('Error fetching exchange data for the given currency and date');
    }
  }
}

export { getStateDataByAcronym };

type RawState = {
  id: number;
  sigla: string;
  nome: string;
  regiao: {
    id: number;
    sigla: string;
    nome: string;
  };
};
